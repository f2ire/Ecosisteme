...
class EnvironmentUnit:
  """The fondamental unit of the environment represented by a square of shape width x length, a volume of 1mm³ and positional coordinates x and y.
...
class TemperatureUnit(EnvironmentUnit):
  ...
  def changeTemperature(self, thermal_flux: float) -> None:
    """Update the actual temperature attribute of the temperature unit according to the thermal flux it is receiving

    Args:
      thermal_flux (float): thermal flux the unit is receiving, in W
    """
    self.temperature = thermal_flux # not the right calculus, convert the thermal flux into actual temperature
    self.adaptTemperatureColor()
...
if __name__ == "__main__":
  ...
  test_temperature_unit.changeTemperature(thermal_flux=330) ; print(test_temperature_unit.temperature == 330) # OK mais fonction pas correcte
