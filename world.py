...
class World:
  ...
  def __init__(self, nb_units_width: int, nb_units_length: int, initial_temperature: float = 298.15, initial_glucose: float = 0) -> None:
    ...
    self.display_tuple = (EnvironmentUnit.display_width  * self.units_on_width, 
                          EnvironmentUnit.display_length * self.units_on_length)
    
    self.environment_grid = EnvironmentGrid(self.units_on_width, self.units_on_length)
    self.temperature_grid = TemperatureGrid(self.units_on_width, self.units_on_length, initial_temperature)
    self.glucose_grid = GlucoseGrid(self.units_on_width, self.units_on_length, initial_glucose)
  ...
  def createUnitDisplayRectangle(self, x_position: int, y_position: int) -> tuple:
    ...
    Returns:
      tuple: _description_
    """
    return (x_position, y_position, EnvironmentUnit.display_width, EnvironmentUnit.display_length)
